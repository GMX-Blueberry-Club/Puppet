...
import { factor, getPnL, getSlotNetPnL, readableDate, readablePercentage, switchMap, timeSince } from "gmx-middleware-utils"
...
export const settledPnlColumn = (puppet?: viem.Address): TableColumn<IPositionMirrorSettled> => ({
  ...
  $$body: map(mp => {
    ...
    return $column(layoutSheet.spacingTiny, style({ textAlign: 'right' }))(
      $pnlValue(pnl),
      $seperator2,
      $text(style({ fontSize: '.85rem' }))(readablePercentage(factor(pnl, collateral) * 100n)),
    )
  })
})
