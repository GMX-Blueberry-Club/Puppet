...
import { awaitPromises, constant, empty, join, map, mergeArray, multicast, now, skipRepeatsWith, startWith, tap } from "@most/core"
import { Stream } from "@most/types"
import { $Baseline, $Link, $Table, $alertTooltip, $arrowRight, $icon, $infoLabel, $infoLabeledValue, $infoTooltipLabel, IMarker, ScrollRequest } from "gmx-middleware-ui-components"
import { getMappedValue, groupArrayMany, readableLeverage, pagingQuery, switchMap, unixTimestampNow, readableDate, readablePercentage, IPricefeedMap, tokenAmountLabel, filterNull, parseReadableNumber, readableFixedUSD30, readableUnitAmount } from "gmx-middleware-utils"
import { IPuppetSubscritpion, accountSettledTradeListSummary, getPuppetDepositAccountKey } from "puppet-middleware-utils"
...
import { $ProfilePerformanceGraph, getUpdateTickList, performanceTimeline } from "../trade/$ProfilePerformanceGraph.js"
...
import { $RouteDepositEditor } from "../route/$RouteDepositEditor"
...
export const $PuppetPortfolio = (config: ITraderProfile) => component((
  ...
  }, combineObject({ settledTrades, openTrades, subscribedRoutes, activityTimeframe: config.activityTimeframe, routeMap, priceLatestMap, pricefeedMap }))
      ...
      $card(layoutSheet.spacingBig, style({ flex: 1, width: '100%' }))(
        $card2(style({ padding: 0, height: screenUtils.isDesktopScreen ? '200px' : '200px', position: 'relative', margin: screenUtils.isDesktopScreen ? `-36px -36px 0` : `-12px -12px 0px` }))(
          switchMap(params => {
            ...
            const timeline = performanceTimeline({ puppet: config.address, positionList: allPositions, pricefeedMap: params.pricefeedMap, tickCount: 100, activityTimeframe: params.activityTimeframe })
            ...
            // })({ })
          }, combineObject({ processData: config.processData, activityTimeframe: config.activityTimeframe, openTrades, settledTrades, pricefeedMap })),
        ),
  ...
  return [
    ...
    $column(layoutSheet.spacingBig)(
      $PuppetProfileSummary({
        settledTradeList, priceLatestMap,
