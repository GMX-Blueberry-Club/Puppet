...
export const $TraderProfile = (config: ITraderProfile) => component((
  ...
  const $metricLabel = $row(style({ color: pallete.foreground, letterSpacing: '1px', fontSize: screenUtils.isDesktopScreen ? '.85rem' : '.75rem' }))
      ...
      $column(layoutSheet.spacingBig, style({ width: '100%', margin: '0 auto' }))(
        ...
        switchMap(params => {
          ...
          return $column(layoutSheet.spacing)(
            $node(style({ display: 'flex', flexDirection: screenUtils.isDesktopScreen ? 'row' : 'column', gap: screenUtils.isDesktopScreen ? '36px' : '16px', zIndex: 10, placeContent: 'center', alignItems: 'center', padding: '0 8px' }))(
              $row(
                $profileDisplay({
        ...
        $column(layoutSheet.spacing)(
          ...
          $card(layoutSheet.spacingBig, style({ flex: 1, width: '100%' }))(
            $card2(style({ padding: 0, position: 'relative', margin: screenUtils.isDesktopScreen ? `-36px -36px 0` : `-12px -12px 0px` }))(
              switchMap(params => {
                const filterStartTime = unixTimestampNow() - params.activityTimeframe
                const traderPos = Object.values(params.processData.mirrorPositionSettled[config.address] || {}).flat().filter(pos => pos.position.blockTimestamp > filterStartTime)
                const openList = Object.values(params.processData.mirrorPositionSlot).filter(pos => pos.trader === config.address).filter(pos => pos.position.blockTimestamp > filterStartTime)
                ...
                return $ProfilePerformanceCard({
                  $container: $column(style({ width: '100%', height: screenUtils.isDesktopScreen ? '200px' : '200px', padding: 0 })),
